        ...
        for h in range(sentence_len):
            ...
            for m in range(sentence_len):
                ...
                if h == m:
                    ...
                    feature_func = torch.cat((lstm_out[h],lstm_out[m])) # feature_func.shape = [2*lstm_hidden_dim]
                    neuron = torch.tanh(self.Linear(feature_func))      # neuron.shape = [mlp_hidden_dim]
